...
public class peerProcess {
  ...
  private class peerConnection extends Thread { // threads so 1 socket doesn't block connections to other peers
    ...
    private Map<Integer, Boolean> handshakeSuccessStatus = new HashMap<Integer, Boolean>();; // Records whether a
                                                                                             // handshake has been
                                                                                             // successfully
                                                                                             // sent/received between
                                                                                             // connected peers.
    ...
    public synchronized void ReadPeerBitfield() {
      try {
        ...
        if (bytesRead == -1) {
          ...
          throw new Exception("Wrong bitfield message type received. Expected: " + message.MessageType.bitfield.value
              + " Received: " + msgTypeBytes[0]);
        }
        ...
        // All bytes may not be readily available in input stream, hence the continuos
        // check for available bytes inside the while loop.
        while (numUnreadPayloadBytes > 0) {
          ...
          if (Math.min(numUnreadPayloadBytes, availableBytes) > 0) {
            ...
            if (bytesRead == -1) {
              ...
              throw new Exception("Wrong bitfield message payload length received. Read " + bytesRead
                  + " bytes from input stream. Expected " + Math.min(numUnreadPayloadBytes, availableBytes)
                  + " bytes.");
            }

            // Store bytes read into message payload bytes.
            System.arraycopy(tempReadBytes, 0, msgPayloadBytes, payloadLength - numUnreadPayloadBytes,
                Math.min(numUnreadPayloadBytes, availableBytes));
    ...
    // TODO(bndalichako): Function below may have issues b/c connectedBitField may
    // be empty.
        ...
        if (Math.min(numUnreadBytes, availableBytes) > 0) {
          ...
          if (bytesRead == -1) {
            ...
            throw new Exception("Wrong number of Bytes received. Read " + bytesRead
                + " bytes from input stream. Expected " + Math.min(numUnreadBytes, availableBytes) + " bytes.");
          }
